        ...
        <b-col cols="7">
          ...
          <b-row>
            <b-col cols="6">
              <p>人物名</p>
              <WordList
                v-bind:list_data="word_data['chara']"
                @update="getData"
              />
            </b-col>
            <b-col cols="6">
              <p>語句</p>
              <WordList
                v-bind:list_data="word_data['word']"
                @update="getData"
              />
            </b-col>
          </b-row>
...
export default {
  ...
  methods: {
    ...
    getData(input) {
      this.input = input
      axios
        .get('https://w3id.org/hi/api/entity/chname/' + this.input)
        .then((response) => {
          this.info_text =
            response.data[0]['http://schema.org/description'][0]['@value']
        })
        .catch((error) => {
          console.log(error)
