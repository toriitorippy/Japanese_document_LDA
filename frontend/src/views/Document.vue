<template>
  <div
    class="topic"
    v-loading="loading"
    element-loading-text="Loading... 10秒ほどお待ちください"
  >
    <b-row>
      <b-col cols="2">
          ...
          <el-submenu :index="getIndex(a)" v-for="a of 10" :key="a">
            ...
            >
              ...
              <el-menu-item-group>
                <el-menu-item
                  :index="getIndex(c)"
                  v-for="c of niki_data[a][b]"
                  :key="getIndex(c)"
                  @click="ClickEvent(c)"
                  >{{ time_data[c] }}</el-menu-item
                >
              </el-menu-item-group>
            </el-submenu>
          </el-submenu>
      ...
      <b-col cols="10">
        <br />
        <h3 style="color: red">
          注：著作権のため公開版は機能が制限されています。
          <br />一部を除きテキスト及びテキストの画像は閲覧することができません。
        </h3>
        ...
        <b-row>
          <b-col cols="5">
            <h3>Topicの割合</h3>
            <DocumentTopicGraph
              v-bind:topic_data="topic_data"
              :total_num="num_topics"
            />
          </b-col>
          <b-col cols="5">
            <el-image :src="topic_data_origin['image']"></el-image>
          </b-col>
        </b-row>
      </b-col>
    </b-row>
...
export default {
  ...
  data() {
    return {
      ...
      niki_data: {
        ...
        2: { 嘉禄２年7月: [9, 11, 12], 嘉禄２年５月: [13], 嘉禄２年６月: [14] },
      },
      time_data: ['嘉禄２年４月１日'],
      topic_data: {
        id: 0,
        image:
          'https://clioimg.hi.u-tokyo.ac.jp/viewer/api/image/idata%2F850%2F8500%2F06%2F1601%2F0001.jpg',
        ...
        topic_rate: [
          0.21544824540615082, 0.0076963393948972225, 0.007696245796978474,
          0.007699206937104464, 0.09019748866558075, 0.6404713988304138,
          0.007696337997913361, 0.0076973289251327515, 0.007700970396399498,
        ...
        text_topic_rate: [
          ...
          [
            8,
            [
              [0, 1.9462515115737915],
              [4, 0.0537404902279377],
            ],
          ],
  ...
  methods: {
    ...
    getData() {
      ...
      const path2 =
        BACKEND_URL + '/text/' + this.num_topics + '/' + this.text_id
      axios
        .get(path2)
        .then((response) => {
          this.topic_data = response.data
