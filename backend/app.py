    ...
    for i in range(len(topic_name_list)):
        ...
        for j in range(len(topic_name_list[0])):
            ...
            if len(tmp.split('、')) > 1:
                ...
                name = tmp
            n_topic_name.append({'id': j, 'name_id': topic_name_list[i][j], 'name': name,
                                'link': 'https://nakamura196.github.io/hi_person/test/person.html?u=https%3A%2F%2Fnakamura196.github.io%2Fhi_person%2Fentity%2Fchname%2F%E8%97%A4%E5%8E%9F%E7%B5%8C%E5%85%89.json'})
...
def get_top_name(total_num=10, num=10):
    ...
    json_load = json.load(json_open)
    word_list = []
    basis = max(json_load['chara'][int(num)][0][1],
                json_load['words'][int(num)][0][1])
    for i in range(30):
        word_dict = {}
...
def get_per_nengo_num(total_num=10, num=10):
    json_open = open(
        './data/json/{}_per_nengo.json'.format(int(total_num)), 'r')
    json_load = json.load(json_open)
    rate_list = []
...
def get_per_month_num(total_num=10, num=10):
    json_open = open(
        './data/json/{}_per_month.json'.format(int(total_num)), 'r')
    json_load = json.load(json_open)
    rate_list = []
    for val in json_load.values():
        rate_list.append(val[int(num)])
...
def get_memo(total_num=10, num=10):
    json_open = open(
        './data/json/memo/{}_memo.json'.format(int(total_num)), 'r')
    json_load = json.load(json_open)
...
def get_top_doc(total_num=10, num=10):
    ...
    score_sorted = sorted(document_list.items(),
    ...
    for i in range(10):
        text_init = data_dict[str(score_sorted[i][0])]['text'][0:30].replace(
            " ", "").replace("\n", "").replace("\u3000", "")
        tmp_dict = {'id': score_sorted[i][0], 'weight': score_sorted[i][1]/top_weight*100,
                    'source': data_dict[str(score_sorted[i][0])]['source'], 'text_init': text_init}
...
def get_per_doc(total_num=10, num=10):
    ...
    document_list = []
    text_open = open(
        './data/json/color_text/{}_text.json'.format(int(total_num)), 'r')
    ...
    for j in range(len(json_load)):
        ...
        tmp_dict = {'id': j, 'text': text_init, 'topic_rate': topic_rate, 'topic_number': topic_number,
                    'text_topic': document_data[1], 'text_topic_rate': document_data[2], 'source': data_dict[str(j)]['source'], 'image': data_dict[str(j)]['image']}
        document_list.append(tmp_dict)
    return jsonify({'data': document_list})
...
def get_per_text(total_num=10, text_id=0):
    ...
    text_dict = {'id': int(text_id), 'text': text_init, 'topic_rate': topic_rate, 'topic_number': topic_number,
                 'text_topic': document_data[1], 'text_topic_rate': document_data[2], 'source': data_dict[str(text_id)]['source'], 'image': data_dict[str(text_id)]['image']}
    return jsonify(text_dict)


@app.route('/word/<total_num>/<word>')
def get_word_info(total_num=10, word=''):
    word_topic_info, related_chara, related_word = load_lda.word_info(
        int(total_num), word)
    topic_number = ['topic' + str(x+1) for x in range(int(total_num))]
    topic_rate = [0]*int(total_num)
    for p in word_topic_info:
        topic_rate[p[0]] = p[1]
    response = {'info': word_topic_info, 'chara': related_chara,
                'word': related_word, 'topic_number': topic_number, 'topic_rate': topic_rate}
    return jsonify(response)
...
@app.route('/upload/<mineType>', methods=['POST'])
def uploadFile(mineType="text/csv"):
    if 'file' not in request.files:
        return 'ファイル未指定'
    fs = request.files['file']
    if mineType == "csv":
        savedir = './data/csv/' + request.files['file'].filename
    if mineType == "xml":
        savedir = './data/xml/' + request.files['file'].filename
    fs.save(savedir)
